...
const pages = Object.entries(markdownModules)
  .map(([file, markdown]) => {
    ...
    return { name, markdown, maturity }
  })
  .sort((a, b) => a.name.localeCompare(b.name))

const nav = pages.map((page) => `<li><a href="#${page.name}">${page.name}</a></li>`).join("\n")
...
const docsBody = pages
  .map((page) => {
    ...
    return `<article id="${page.name}"><p><strong>Maturity:</strong> ${page.maturity}</p>${demoSection}<pre>${escapeHtml(page.markdown)}</pre></article>`
  })
  .join("\n")
